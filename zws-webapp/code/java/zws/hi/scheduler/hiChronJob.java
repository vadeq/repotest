...
//  public hiChronJob(ChronJob j) { super(null); job = j; render();}
  ...
  public void adapt(Object o) { adapt((ChronJob) o);}

  public void adapt(ChronJob j) {
    job = j;
    setProcessName(j.getProcessName());
    setID(j.getName());
    setJobName(j.getName());
    setPeriodHours(""+j.getPeriodHours());
    setPeriodMinutes(""+j.getPeriodMinutes());
    setStartHour(""+j.getStartTime().get(Calendar.HOUR));
    setStartMinute(""+j.getStartTime().get(Calendar.MINUTE));
  }
...
*/
  ...
  public Collection getAvailableProcessors() throws Exception { 
   String serverName = Properties.get(Names.CENTRAL_SERVER);
   Collection c = ProcessorClient.getAllProcessors(serverName);
   if (null==c) return new Vector();
   Collection names = new Vector();
   Iterator i = c.iterator();
   while (i.hasNext()) { 
     names.add(((Processor)i.next()).getName());
   }
   return names;
  }
