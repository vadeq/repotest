...
public class Ilink3QxSandboxInvoker {

  public static QxXML executeQx(String envRoot, String opType, QxXML instruction, boolean acquireLicense) throws Exception {
    Object token = null;
    String path = "/zws/env/ptc/" + envRoot;
    String dotProIPath = path + "/LDB/.proi";
    File dotProI = new File(dotProIPath);
    if (!dotProI.exists()) return new QxXML("<exception message='missing .proi folder' location='"+dotProIPath+"'/>");
    ...
    if (acquireLicense) token = tkPool.takeToken();
    try {
      getNewCount();
