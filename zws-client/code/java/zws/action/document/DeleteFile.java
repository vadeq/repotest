...
@author: athakur
@version: 1.0
Created on January 19, 2004, 12:25 AM
...
public class DeleteFile extends ActionBase {

  public void execute() throws Exception {
    boolean deleted;
    if (null==getString("location")) deleted = deleteLocalFile();
    else deleted = deleteRemoteFile();
    setResult(new Boolean(deleted));
  }
    
  public boolean deleteLocalFile() throws Exception {  
  ...
  public boolean deleteRemoteFile() throws Exception {  
    String server = getString("serverName");
    if (null==server) server = grabOrigin().getServerName();
    String file = getRequiredString("filename");
    String type = getString("fileType");
    if (null!=type) file = FileNameUtil.convertType(file, type);
    return DocumentAccess.deleteFile(server, getRequiredString("location"), file);
  }
