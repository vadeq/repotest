...
public class PromoteFormSearchResultHandler extends IntralinkResultHandler {
  ...
  private boolean formIsApproved(RTPForm f) {
    ...
    while (i.hasNext() && approved) {
      ...
      //String releaseLevel = data.get(Ilink3Constants.RELEASE_LEVEL);
//this should be uncommented once getSequenceForReleaseLevel is implemented
//      if ( getIlink3Svc().getSequenceForReleaseLevel(releaseLevel) < 
//           getIlink3Svc().getSequenceForReleaseLevel(f.getPromoteTo()) ) approved=false;
    }
    return approved;
  }
