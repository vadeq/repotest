...
public class hiZWSPublish2Agile  extends hiZWSReport {
  ...
  public I2AMetadataAdapter findSourceDesign(Metadata agileData) throws Exception, NotSynchronized {
	  ...
	  I2AMetadataAdapter adapter = new I2AMetadataAdapter();
	  adapter.setMetadataFields (new String[] {"name", "branch", "rev", "ver"});
	  adapter.adapt(source);
	  return adapter;
	}

  public I2AMetadataAdapter adaptSourceDesign(Origin ilinkOrigin) throws Exception, NotSynchronized {
  ...
  public String publishCADDocuments() {      
	  Map agileMappings = MapUtil.getMapFromMap(hiZWSDemo.agileMappings, "CAD Document");
  ...
  public String publishCADModels() {
    Map agileMappings = MapUtil.getMapFromMap(hiZWSDemo.agileMappings, AGILE_CLASS_CAD_PART);
    if (agileMappings.isEmpty()) {
  ...
  public String orderInitialRelease() {
    ...
    I2AMetadataAdapter m = (I2AMetadataAdapter)getChosenItems().iterator().next();
...
*/
  ...
  protected Metadata mapAttributes(MetadataBase data, String agileClassName) throws Exception {
	  ...
	  while (i.hasNext()) {
	    ...
	    val = data.get(key);
	    if (number.startsWith(getTempPartNumberPrefix())) {
  ...
  public String revUpdate() {
    ...
    createNewECO(getECOUpdateName(), getECOUpdateWorkflowName());
    ...
    {} //System.out.println("Updating to ECO: " + eco.getNumber());
	  ...
	  */
	  ...
	  try {
	    ...
	    if ("YES".equalsIgnoreCase(getAttachNativeFile()) || "YES".equalsIgnoreCase(getAttachIGESImage()) || "YES".equalsIgnoreCase(getAttachSTEPImage())) publishViewableImages(c);	  
	    agile().moveToNextStatus(eco.getNumber(), getAuthentication());
	    if (null!=newParts && !newParts.isEmpty()) {
	      bindIntralinkPartNumbers(loadedPartNumbers);
	      Iterator i = getChosenItems().iterator();
	      
	      I2AMetadataAdapter adapter;
	      Metadata data;
	      String partnumber;
	      while (i.hasNext()) {
	        adapter = (I2AMetadataAdapter) i.next();
  ...
  /*
  public void bindCiscoEngineeringContainer(AgileECO eco, Map validCiscoPartNumbers) throws Exception {
...
//    pendingECOs.putAll(agile().findPendingECOs(engContainerNames));
  ...
  */
  ...
  public String zwsRevUpdate() {
    //+++for initial items, just publishWIP
    selectedECO=null;
    return orderZWSRevUpdate();
  }
  ...
  public String orderZWSRevUpdate() {
    ...
    //else {} //System.out.println("eco defined " + eco.getNumber());
	  ...
	  */
      ...
      Map validPartNumbers = new HashMap();
      Map invalidPartNumbers = new HashMap();
	    //walk each structure and categorize all parts  
	    //check for multiple versions of same name in different places in the structure
	    try {
    	  //categorizeChanges(c, parts, unchangedParts, newParts, renamedParts, updatedParts, lastPublishedOrigins, lastPublishedModelOrigins, lastPublishedDocumentOrigins, lastPublishedPartNumberOrigins, undeterminedParts);
	    ...
	    Collection loadedPartNumbers=new Vector();
	    if (null!=newParts && !newParts.isEmpty()) {
	      establishPartNumbers(newParts, validPartNumbers, invalidPartNumbers, getTempPartNumberPrefix());

	      //Map proposedPartNumbers = proposePartNumbers(newParts);
	      //newPartNumbers = agile().establishNewPartNumbers(proposedPartNumbers, AGILE_CLASS_CAD_PART, null);
	      //add -000 to the end of the new partnumbers
	      //newPartNumbers = formatNewPartNumbers(newParts, newPartNumbers);
	      PrintUtil.print(validPartNumbers);
	      PrintUtil.print(invalidPartNumbers);
	      newPartNumbers.putAll(validPartNumbers);
	      newPartNumbers.putAll(invalidPartNumbers);
	      //update AGILE_NUMBER and ILINK_PART_NUMBER metadata atts
	      loadNewPartNumbers(newParts.values(), newPartNumbers);
	    ...
	    while (a.hasNext()) {
		    m = (Metadata) a.next();
			  {} //System.out.println(m.getMetadataBase().get(ATT_AGILE_NUMBER));
			  if (m.getMetadataBase().get(ATT_AGILE_NUMBER).startsWith(getTempPartNumberPrefix())) continue;
		    changedItem = new AffectedItem();
			  changedItem.setItemNumber(m.getMetadataBase().get(ATT_AGILE_NUMBER));
	      changedItems.add(changedItem);
	    }
	    ...
	    */
        ...
        data = (Metadata)i.next();
	      number = data.get(ATT_AGILE_NUMBER);
	      if (number.startsWith(getTempPartNumberPrefix())) tmpChanges.add(data);
	    ...
	    while (i.hasNext()) {
	      data = (Metadata)i.next();
	      if (data.get(ATT_AGILE_NUMBER).startsWith(getTempPartNumberPrefix())) {
	        if ("ALL".equalsIgnoreCase(getAttachURL())) attachURL(data.get(ATT_AGILE_NUMBER), data.getOrigin());
    	    if ("ALL".equalsIgnoreCase(getAttachNativeFile())) attachNative(data.get(ATT_AGILE_NUMBER), data.getOrigin());	          
    	    if ("ALL".equalsIgnoreCase(getAttachIGESImage())) attachIGES(data.get(ATT_AGILE_NUMBER), data.getOrigin());	          
    	    if ("ALL".equalsIgnoreCase(getAttachSTEPImage())) attachSTEP(data.get(ATT_AGILE_NUMBER), data.getOrigin());	          
	      }
	      else {
	        if ("ALL".equalsIgnoreCase(getAttachURL())) attachURL(data.get(ATT_AGILE_NUMBER), activeECO().getNumber(), data.getOrigin());
	        if ("ALL".equalsIgnoreCase(getAttachNativeFile())) attachNative(data.get(ATT_AGILE_NUMBER), activeECO().getNumber(), data.getOrigin());
    	    if ("ALL".equalsIgnoreCase(getAttachIGESImage())) attachIGES(data.get(ATT_AGILE_NUMBER), activeECO().getNumber(), data.getOrigin());
    	    if ("ALL".equalsIgnoreCase(getAttachSTEPImage())) attachSTEP(data.get(ATT_AGILE_NUMBER), activeECO().getNumber(), data.getOrigin());
	      }
	    }
	    i = c.iterator();
	    while (i.hasNext()) {
	      data = (Metadata)i.next();
	      if (data.get(ATT_AGILE_NUMBER).startsWith(getTempPartNumberPrefix())) {
	        if ("TOP".equalsIgnoreCase(getAttachURL())) attachURL(data.get(ATT_AGILE_NUMBER), data.getOrigin());
    	    if ("TOP".equalsIgnoreCase(getAttachNativeFile())) attachNative(data.get(ATT_AGILE_NUMBER), data.getOrigin());	          
    	    if ("TOP".equalsIgnoreCase(getAttachIGESImage())) attachIGES(data.get(ATT_AGILE_NUMBER), data.getOrigin());	          
    	    if ("TOP".equalsIgnoreCase(getAttachSTEPImage())) attachSTEP(data.get(ATT_AGILE_NUMBER), data.getOrigin());	          
	      }
	      else {
	        if ("TOP".equalsIgnoreCase(getAttachURL())) attachURL(data.get(ATT_AGILE_NUMBER), activeECO().getNumber(), data.getOrigin());
	        if ("TOP".equalsIgnoreCase(getAttachNativeFile())) attachNative(data.get(ATT_AGILE_NUMBER), activeECO().getNumber(), data.getOrigin());
    	    if ("TOP".equalsIgnoreCase(getAttachIGESImage())) attachIGES(data.get(ATT_AGILE_NUMBER), activeECO().getNumber(), data.getOrigin());
    	    if ("TOP".equalsIgnoreCase(getAttachSTEPImage())) attachSTEP(data.get(ATT_AGILE_NUMBER), activeECO().getNumber(), data.getOrigin());
	      }
	    }

	    //if autorelease - then eco must be moved to next status for agile to auto release it
	    // add to PfP to check if eco is auto release and should be moved to next status
	    //if (!changedItems.isEmpty() || validPartNumbers.isEmpty()) agile().moveToNextStatus(activeECO().getNumber(), getAuthentication());

	    if (null!=activeECO()) updateECOStatus(activeECO());
  ...
  public String getECOUpdateName() { return ecoUpdateName; }
  public void setECOUpdateName(String s) { ecoUpdateName=s; }
  
  public String getECOUpdateWorkflowName() { return ecoUpdateWorkflowName; }
  public void setECOUpdateWorkflowName(String s) { ecoUpdateWorkflowName=s; }


  public String getECOLibraryName() { return ecoLibraryName; }
  public void setECOLibraryName(String s) { ecoLibraryName=s; }
  
  public String getECOLibraryWorkflowName() { return ecoLibraryWorkflowName; }
  public void setECOLibraryWorkflowName(String s) { ecoLibraryWorkflowName=s; }
  
  //put into policy for publish - or config files
  private String ecoUpdateName= "Intralink ECO";
  //private String ecoUpdateWorkflowName = "Intralink Workflow Auto Release";
  private String ecoUpdateWorkflowName = "Default Change Orders";
  private String ecoLibraryName= "Intralink ECO";
  private String ecoLibraryWorkflowName = "Intralink Standard Lib Workflow";
