...
public class IntralinkParserPub {

	Logger logger;
	String xmlFile = "";
	String parseMode = "";
	AgileConnectorPub connector;
	InputStream xmlStream;
	SAXBuilder builder;
	Document xmlDoc;
	XMLMapping mapper = null;
	
	public IntralinkParserPub (String mode, String file, Logger log, AgileConnectorPub ac) {
		this.xmlFile = file;
		this.parseMode = mode;
		logger = log;
		connector = ac;
	...
	public IntralinkParserPub (String file, Logger log, AgileConnectorPub ac) {
		this.xmlFile = file;
		logger = log;
		connector = ac;
		builder = new SAXBuilder();
	...
	public IntralinkParserPub (Logger log, AgileConnectorPub ac) {
		logger = log;
		connector = ac;
		builder = new SAXBuilder();
	}
	
	public IntralinkParserPub (String file, Logger log, AgileConnectorPub ac, XMLMapping map) {
		this.xmlFile = file;
		logger = log;
		connector = ac;
		builder = new SAXBuilder();
	...
	public IntralinkParserPub (Logger log, AgileConnectorPub ac, XMLMapping map) {
		logger = log;
		connector = ac;
		builder = new SAXBuilder();
		mapper = map;
	...
	public XMLItem[] parseItems (String createPartsFlag) {
		...
		try {
			...
			while (itemIter.hasNext()) {
				...
				XMLItem publishedItem = recurseXMLItems(item, createPartsFlag);
	...
	public XMLItem[] parseItems (InputStream xmlStream, String createPartsFlag) {
		boolean itemExists = false;
		XMLItem[] publishedItems = null;;
		try {
			//xmlDoc = builder.build(xmlFile);
	...
	public XMLItem recurseItems (Element item, String createPartsFlag) {
		...
		try {
			...
			if (!itemExists) {
				...
				//check flag
				if (createPartsFlag.equalsIgnoreCase("YES")) {
			...
			if (agileItem != null) {
				...
				while (itemIter.hasNext()) {
					...
					pubChild = recurseItems(childItem, createPartsFlag);
	...
	public XMLItem recurseXMLItems (Element item, String createPartsFlag) {
		IItem agileItem = null;
		boolean itemExists = false;
		Vector childNumbers = null;
		XMLItem pubItem = null;
		...
		try {
			...
			if (agileItem != null) {
				...
				while (itemIter.hasNext()) {
					XMLItem pubChild = null;
					Element childItem = (Element)itemIter.next();
					pubChild = recurseXMLItems(childItem, createPartsFlag);
					pubItemChildren[childrenIndex] = pubChild;
					//logger.info("childrenIndex : " + childrenIndex);
					childrenIndex++;
				}
