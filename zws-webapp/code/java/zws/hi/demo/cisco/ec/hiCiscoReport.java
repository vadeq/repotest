...
public class hiCiscoReport extends hiReport {
  ...
  public boolean idChoosesItem(String id, Object item) {
    CiscoMetadataAdapter adapter = (CiscoMetadataAdapter) item;
  ...
  public void publishTMPBOMStructure(Collection metadataList) throws Exception {
    ...
    parents.addAll(extractTMPCiscoParents(metadataList));
    agile().replaceFirstLevelBOM(parents, getAuthentication());
  }

  private Collection extractTMPCiscoParents(Collection parentList) throws Exception {
    Collection parents = new Vector();
    ...
    while (i.hasNext()) {
      ...
      if (number.startsWith(getTempCiscoPartNumberPrefix()) && data.hasSubComponents()) parents.add(data);
    }
    Collection c = new Vector();
    i = parents.iterator();
    while(i.hasNext()) {
      data = (Metadata) i.next();
      c.addAll(extractTMPCiscoParents(data.getSubComponents()));
    }
    parents.addAll(c);
    return parents;
  }
  ...
  public void createChosenItems(String agileClassName) throws Exception {
    ...
    if (agileClassName.equals("CAD Part") && ("YES".equalsIgnoreCase(getAttachNativeFile()) || "YES".equalsIgnoreCase(getAttachIGESImage()))) {    
      Map agileMappings = MapUtil.getMapFromMap(hiCiscoDemo.agileMappings, agileClassName);
  ...
  public void renameInSyncLog(CiscoMetadataAdapter data) throws Exception {
    String newName;
  ...
  protected Metadata map(Metadata data, String agileClassName) throws Exception {
    ...
    while (i.hasNext()) {
      ...
      if (agileClassName.equals("CAD Part") && hiCiscoDemo.isExcluded(kid.getName())) continue;
  ...
  public String generateNextPartNumberBase() throws Exception {
    ...
    MetadataAdapter adapter;
	  ...
	  while (i.hasNext()) {
	    ...
	    if (!hiCiscoDemo.isExcluded(adapter.getName())) {
  ...
  protected void numberPart(Map parts, Metadata data) throws Exception{
    if (hiCiscoDemo.isExcluded(data.getName())) return;
    String number;
  ...
  public IntralinkOrigin findPastNamedOrigin(IntralinkOrigin o, String partnumber){
    ...
    Map emptyPartNumbers = new HashMap();
	  ...
	  while (i.hasNext()) {
	    data= (Metadata)i.next();
	    if (!hiCiscoDemo.isExcluded(data.getName())) {
  ...
  protected void findUnpublishedPartNumbers(Map emptyPartNumbers, Metadata data) {
    ...
    while (i.hasNext()) {
      kid = (MetadataSubComponent) i.next();
      if (hiCiscoDemo.isExcluded(kid.getName())) continue;
  ...
  protected void establishCiscoPartNumbers(Map emptyPartNumbers, Map validCiscoPartNumbers, Map invalidCiscoPartNumbers, String tempNoPrefix) {
	  ...
	  while (i.hasNext()) {
	    ...
	    name = data.getName();
	    if (null!=validCiscoPartNumbers.get(name)) continue;
	    if (null!=invalidCiscoPartNumbers.get(name)) continue;
	    proposedPartNumber = proposeCiscoPartNumber(data, tempNoPrefix);
	    if (proposedPartNumber.startsWith(tempNoPrefix)) invalidCiscoPartNumbers.put(name, proposedPartNumber);
	    else validCiscoPartNumbers.put(name, proposedPartNumber);
	  }
	  return;
  }

  public String proposeCiscoPartNumber(Metadata data, String tempNoPrefix) {
    ...
    try {
      ...
      proposal = CISCO_PART_NUMBER_PREFIX  + basename.toUpperCase() + "_" + type;
    }
...
/*
  ...
  */
  ...
  private AgileAccess agile() throws Exception {
   ...
   int idx = downloadID.indexOf(o.NA);
  ...
  public String getTempCiscoPartNumberPrefix() throws Exception {
    if (null==tmpCiscoPartNumberPrefix) {
      tmpCiscoPartNumberPrefix = agile().generateNextPartNumber(AGILE_CLASS_CAD_PART, null);
      tmpCiscoPartNumberPrefix = tmpCiscoPartNumberPrefix.substring(0, tmpCiscoPartNumberPrefix.indexOf("_"));
      {} //System.out.println("Using temp Part number prefix " + tmpCiscoPartNumberPrefix);
    }
    return tmpCiscoPartNumberPrefix;
  }

  private String tmpCiscoPartNumberPrefix=null;
  protected static String CISCO_PART_NUMBER_PREFIX = "MWF_";
