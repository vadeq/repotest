...
public class AgileConnector {
	...
	public AgileConnector(String test) {
	...
	public AgileConnector(String user, String pass, String xmlFile) {
	...
	public XMLItem[] processXML (String xmlFile) {
		XMLItem[] pubItems = null;
		IntralinkParser parser = new IntralinkParser(xmlFile, logger, this);
		//IntralinkParser parser = new IntralinkParser(xmlFile, logger, this, mapper);
		pubItems = parser.parseItems();
		return pubItems;
	}
	
	public XMLItem[] processXML (InputStream xmlStream) {
		XMLItem[] pubItems = null;
		IntralinkParser parser = new IntralinkParser(logger, this);
		//IntralinkParser parser = new IntralinkParser(xmlFile, logger, this, mapper);
		pubItems = parser.parseItems(xmlStream);
		return pubItems;
	}
			...
			while (attTableIter.hasNext()) {
				...
				if (folderFileName.equals(file3)) {
					...
					for (int i = 0; i < versionValues.length; i++) {
						{}//Logwriter.printOnConsole(i + ", " +
										//((IAgileList)versionValues[i]).getValue());
	...
	public static void main(String[] args) 
	{
		...
		if (args.length == 1) { 
			...
			//System.exit(0);
			AgileConnector ac = new AgileConnector();
			connected = ac.init();
			...
			IntralinkParser parser = new IntralinkParser(xmlFile, logger, ac);
			parser.parseItems();
			//ac.tester();
			ac.disconnect();
		}
